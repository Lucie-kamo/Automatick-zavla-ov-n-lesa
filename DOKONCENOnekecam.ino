...
void vykresliPrerusit(){
  u8g.firstPage();
  while(u8g.nextPage()){
    u8g.setFont(u8g_font_helvB14);
    String prerusit = "PRERUSENO!";
    textSirka = u8g.getStrWidth(prerusit.c_str()); 
    xPos = (128 - textSirka) / 2;  
    u8g.setPrintPos(xPos, 30);      
    u8g.print(prerusit);

    u8g.setFont(u8g_font_7x14);
    String chybiVoda = "malo vodicky..";
    textSirka = u8g.getStrWidth(chybiVoda.c_str());
    xPos = (128 - textSirka) / 2;  
    u8g.setPrintPos(xPos, 50);      
    u8g.print(chybiVoda);
    u8g.drawLine(0,61,128,61);
...
void loop() {
  ...
  if(stav == HIGH && odpocetAktivni){
    ...
    if(pamatujRezim != -1) {
        ...
        aktualniRezim = 6; // Výchozí menu zálivky
    }
  }
