...
void vykresliGraf(){
  ...
  while(oled_1.nextPage()){
    ...
    for(int i = 0; i < pocetSloupcu; i++){
      ...
      if(hodnota > 15) hodnota = 15;
      int vyskaSloupce = hodnota * vyska / 15;
      if(vyskaSloupce > 0) {
        oled_1.drawBox(x, spodniOkraj - vyskaSloupce, sirkaSloupce, vyskaSloupce);
      }
      ...
      int cisloMesice = (datumCas.month - (pocetSloupcu - i) + 12) % 12;
      //int cisloMesice = ((datumCas.month - 1) - (pocetSloupcu - 1 - i) + 12) % 12;

      //int hodina = (datumCas.hour - (pocetSloupcu-1-i) + 24) % 24;
...
void vykresliGraf() {
  ...
  while(oled_1.nextPage()){
    ...
    for(int i = 0; i <= 3; i++){
      ...
      oled_1.drawLine(20, y, 120, y);
      oled_1.setPrintPos(0, y + 3);
      oled_1.print(i * 5); // Jednoduché měřítko 0, 3, 6, 9
    }
    ...
    for(int i = 0; i < pocetSloupcu; i++){
      ...
      if(h > 0) {
        oled_1.drawBox(x, spodniOkraj - h, 20, h);
      }
      
      // Popisky pod sloupci
      int cisloMesice = (datumCas.month - (pocetSloupcu - 1 - i) + 11) % 12; 
      oled_1.setPrintPos(x, spodniOkraj + 12);
      oled_1.print(nazvyMesicu[cisloMesice]);
    }
  }
}
...
void vykresliStatMinut(){
  ...
  while(oled_1.nextPage()){
    ...
    //String velkeCislo = "126 minut(y)"; //NATVRDO
    textWidth = oled_1.getStrWidth(velkeCislo.c_str()); // šířka textu v pixelech
    xPos = (128 - textWidth) / 2;  
    oled_1.setPrintPos(xPos, 30);      // vertikální pozice
    oled_1.print(velkeCislo);
...
void vykresliUlozeno(){
  ...
  while(oled_1.nextPage()){
    ...
    textWidth = oled_1.getStrWidth(ulozeno.c_str()); // šířka textu v pixelech
    xPos = (128 - textWidth) / 2;  
    oled_1.setPrintPos(xPos, 30);      // vertikální pozice
    oled_1.print(ulozeno);

    oled_1.setFont(u8g_font_7x14);
    String jupi = "jupii";
    textWidth = oled_1.getStrWidth(jupi.c_str());
    xPos = (128 - textWidth) / 2;  
    oled_1.setPrintPos(xPos, 50);      // pod číslem
    oled_1.print(jupi);
    // rovná čára (od souřadnic -> do souřadnic)
...
void loop() {
  ...
  //tim si nejsem jista
  if(odpocetAktivni){
    //mozna budu muset presunout do loop!!!

    celkemOdpocet = hodinZ*3600 + minutZ*60 + sekundZ;
    //casStart = millis();
    //spocitat sekundy
    s =celkemOdpocet - (millis() - casStart) / 1000;
  ...
  if(ted - posledniVypis >= 1000){
    ...
    if(odpocetAktivni){
        ...
        if(s <= 0){ // konec odpočtu
            odpocetAktivni = false;
            digitalWrite(LED_ZELENA, LOW);

            // navrat do ulozeneho režimu
            aktualniRezim = pamatujRezim;
        }
    }
    */
